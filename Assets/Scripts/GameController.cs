...
public class GameController : MonoBehaviour {
    ...
    public float spawnCopWaitMax, spawnCopWaitMin;
    public float spawnAmbulanceWaitMax, spawnAmbulanceWaitMin;
    public float spawnTurretWaitMax, spawnTurretWaitMin;
	...
	void Start ()
    {
        ...
        StartCoroutine(SpawnCopCarWaves());
        StartCoroutine(SpawnAmbulanceWaves());
	}
    ...
    IEnumerator SpawnGarbageWaves()
    {
        ...
        while (true)
        {
            ...
            float spawnPositionX = Random.Range(-garbageTruckSpawnValues.x, garbageTruckSpawnValues.x);
    ...
    IEnumerator SpawnCopCarWaves()
    {
        ...
        while (true)
        {
            ...
            Vector3 spawnPosition = new Vector3(
                    ...
                    garbageTruckSpawnValues.z);                                             // spawn it at the Z from the Vector3
            Quaternion spawnRotation = Quaternion.identity;                                 // sets the rotation of the spawn
            Instantiate(copCar, spawnPosition, spawnRotation);                        // spawn the truck

            // set the wait time to a random value between the min and max spawn time
            float spawnWait = Random.Range(spawnCopWaitMin, spawnCopWaitMax);
    ...
    /// </summary>
    /// <returns></returns>
    IEnumerator SpawnAmbulanceWaves()
    {
        // wait for the inital start offset before spawning
        yield return new WaitForSeconds(startWait * 20);
        ...
        while (true)
        {
            ...
            Vector3 spawnPosition = new Vector3(
                    ...
                    garbageTruckSpawnValues.z);                                             // spawn it at the Z from the Vector3
            Quaternion spawnRotation = Quaternion.identity;                                 // sets the rotation of the spawn
            Instantiate(ambulance, spawnPosition, spawnRotation);                        // spawn the truck

            // set the wait time to a random value between the min and max spawn time
            float spawnWait = Random.Range(spawnAmbulanceWaitMin, spawnAmbulanceWaitMax);
    ...
    IEnumerator SpawnTurretWaves()
    {
        ...
        while(true)
        {
            ...
            float spawnNextWave = Random.Range(spawnTurretWaitMin, spawnTurretWaitMax);
            yield return new WaitForSeconds(spawnNextWave);
        }
